...
Los $V_{j}$ son los espacios de \textbf{aproximación} ya que si proyectamos ortogonalmente la señal de interés en este espacio obtenemos una señal $A_{j}(t)$ que mejora a medida que j decrece, es decir que $A_{j}(t)$ tiende a la señal a medida que j tiende a $-\infty$. A su vez $W_{j}$ es llamado conjunto de \textbf{detalle} debido a que la proyección ortogonal de la señal en este espacio brinda la señal $D_{j}(t)$ encargada de corregir a $A_{j}(t)$ para mejorar la aproximación a la señal original.[mischel misti pagina 45].
A raiz de esto podemos definir la señal de aproximacion $A_{j-1}(t)$ como :  
...
Antes de proseguir se definirá el significado del subindice j, presente en el análisis anterior. Este subindice hace referencia al nivel de descomposición del espacio de aproximación en su espacio de aproximación y detalles en un nivel superior. Si reescribimos la eq.\ref{Central} de la siguiente forma:
...
\label{eq.Vj}
\end{equation}
A partir de esto se deduce que es posible descomponer a un espacio de aproximación de nivel J en la suma de un espacio de aproximación en un nivel inferior y los espacios de detalle correspondientes a ese nivel. Por lo tanto el nivel de descomposición se relaciona con la cantidad de detalles que se utilizara para estudiar la señal de interés. Resumiendo eq.\ref{eq.Vj} expresa que es posible pasar de un nivel de resolución $j$ a cualquier nivel superior sumando los sucesivos detalles de los niveles inferiores. Esto mejora la aproximación de la señal a medida que el nivel $J$ decrece. 
Se observa que cualquier nivel de resolución $V_j$ se puede expresar como:
...
Finalmente, como el espacio $V_j$ se relaciona con la señal de aproximacion $A_J(t)$ y el $W_j(t)$ con la señal de detalle $D_j(t)$:
...
Teniendo en mente la idea del filtrado, y aún con el inconveniente de que el producto interno para calcular los coeficientes (eq.\ref{Beta} y \ref{Alfa}) requiere un alto costo computacional, se plantea una estrategia basada en un algoritmo que relacione las bases ortonormales de wavelets y los bancos de filtros \cite{CT}. Dicho algoritmo permite calcular los coeficientes $a^{j}$ y $d^{j}$ a partir de los coeficientes de aproximación de una escala anterior de mayor resolución, es decir los $a^{j-1}$. Para llegar a esta relación se parte de la eq.\ref{NivApDet} donde:
...
Se observa que estos coeficientes son productos internos de funciones ya conocidas, la función wavelet y su función de escala. Estos valores ya fueron calculados y se los asume como conocidos, por lo cual si se consideran como los coeficientes de filtros se puede definir a $LoD=\{[LoD]_{n}\}_{n \in \mathbb{Z}}$ y a $HiD=\{[Hid]_{n}\}_{n \in \mathbb{Z}}$ como el filtro pasabajos y un filtro pasaaltos respectivamente.
...
Como se mencionó anteriormente, las señales $a^{j}$ y $d^{j}$ tienen una dimensión menor a la original que se desea reconstruir, por lo que es necesaria ``rellenarlas" para que tengan la dimensión correcta.
...
\end{equation}
\end{itemize}
Donde $n $y $m $ son las dimensiones de la imagen, $I$ es la imagen original sin ruido, $K$ es la imagen a la cual se le quiere calcular el estadístico. Para el SSIM $\mu_I$ es la media de la imagen original y $\mu_K$ es la media de la imagen a comparar. De manera analóga para $\sigma_I$ y $\sigma_K$. El valor de 
...
\item[4)] Corroborar funcionamiento con imágenes con ruido no sintético.
\end{itemize}
...
Para corroborar este resultado, se presentan las tablas comparativas de las distintas combinaciones de parámetros descritos en el item 4-iii en la sección \ref{asd}. Los estadísticos que se utilizaron para comparar son el PSNR y el SSIM. El PSNR define la relación entre la máxima energía posible de una señal y el ruido que la afecta. Mientras menor es su valor, mayor es la cantidad de ruido en la imagen. Por otra parte, el SSIM mide la similitud entre dos imágenes. El resultado varía entre $0$ y $1$, donde SSIM = 1 indica que las imágenes son iguales. La ventaja de este estadístico recae en que los resultados coinciden con la percepción del ojo humano, lo que no sucede con el PSNR.
...
\end{tabular}
\label{tabla-levels}
\caption{PSNR utilizando wavelet Sym8, umbral normalshrink, método soft.}
\end{table}
...
\caption{SSIM utilizando wavelet Sym8, umbral normalshrink, método soft.}
\end{table}
...
\begin{tabular}{lrrr}
\toprule
{imagen\textbackslash método } &  noise &   soft &   hard \\
\midrule
...
\end{tabular}
\label{tabla-modos}
\caption{PSNR utilizando wavelet Sym8, dos niveles de descomposición, umbral normalshrink.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrr}
...
\caption{SSIM utilizando wavelet Sym8, dos niveles de descomposición, umbral normalshrink.}
\end{table}
\begin{figure}
        \centering
...
\caption{PSNR utilizando wavelet Sym8, dos niveles de descomposición, método soft.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrrrrr}
...
\caption{SSIM utilizando wavelet Sym8, dos niveles de descomposición, método soft.}
\end{table}
\begin{figure}[H]
        \centering
        \begin{subfigure}[H]{0.2\textwidth}
            \centering
            \includegraphics[width=\textwidth]{Fig/Wav/2_normal_soft_haar_Wave}
            \caption[Lenna.]%
           {{\small Wavelet haar.}}    
            \label{Lenna}
        \end{subfigure}
     %   \hfill
        \begin{subfigure}[H]{0.2\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{Fig/Wav/2_normal_soft_db4_Wave}
            \caption[]%
            {{\small Wavelet db4.}}    
            \label{House}
        \end{subfigure}
        \begin{subfigure}[H]{0.2\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{Fig/Wav/2_normal_soft_sym8_Wave}
            \caption[]%
            {{\small Wavelet sym8.}}    
            \label{House}
        \end{subfigure}
       \vskip\baselineskip
        \label{fig-imagenes-filtradas-modes}
        \caption[ The average and standard deviation of critical parameters ]
         {\small Imágenes filtradas con distintas wavelet.} 
\end{figure}
\begin{figure}
        \centering
        \begin{subfigure}[H]{0.22\textwidth}
...
\begin{tabular}{lrrrr}
\toprule
{imagen\textbackslash wavelet} &  noise &   haar &    db4 &   sym8 \\
\midrule
...
\caption{PSNR utilizando dos niveles de descomposición,umbral normalshrink, método soft.}
...
\end{table}
Al comparar el PSNR se obseva que el filtro de Wiener es mejor en los tres casos, pero al ver la Tab.\ref{tab.SSIM-filters} el filtrado con wavelet es mejor.

\subsection{Aplicación a imágenes reales}

Por último, con el fin de verificar el funcionamiento del filtrado con imágenes reales(es decir que el ruido presente en ellas no se agregó artificialmente)
